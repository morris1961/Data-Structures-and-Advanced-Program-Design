...
public:
  ...
  virtual bool contains(int len, const ItemType entries[]) const = 0;
...
public:
  ...
  bool contains(int len, const ItemType entries[]) const;
...
bool ArrayBag<ItemType>::removeAll(const ItemType& anEntry)
{
  int locatedIndex = getIndexOf(anEntry);
  bool canRemoveItem = (locatedIndex > -1);
  while(locatedIndex > -1)
  {
  	itemCount--;
    items[locatedIndex] = items[itemCount];
    locatedIndex = getIndexOf(anEntry);
  }
...
template<typename ItemType>
bool ArrayBag<ItemType>::contains(int len, const ItemType entries[]) const
{
  for(int i = 0; i < len; i++)
  {
  	bool isExsist = contains(entries[i]);//(getIndexOf(entries[i]) > -1);
  	if(!isExsist)
  		return false;
  }
  return true;
